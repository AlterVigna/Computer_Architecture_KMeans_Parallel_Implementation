...
__global__ void assignPointsToClusterKernel(float* pointsArray, Point* centroids, int* membership, Point* sums, int* counts) {
    ...
    if (idx < DATASET_SIZE) {
        ...
        for (int i = 0; i < K; ++i) {
            float distance = 0.0f;
            for (int j = 0; j < DIM; ++j) {
                distance += (pointsArray[idx + DATASET_SIZE * j] - shared_data[i + j * K]) * (pointsArray[idx + DATASET_SIZE * j] - shared_data[i + j * K]);
            }
        ...
        for (int dim = 0; dim < DIM; dim++) {
            atomicAdd(&sums[minIndex].coordinate[dim], pointsArray[idx + DATASET_SIZE * dim]);
        }
...
int main() {
    ...
    printf("----- GPU KMeans Flattened 1D Array ----- \n");
    ...
    //Definide in KMeans.h used to verify the correct loading of the dataset
    ...
    //******NEW DATA STRUCTURE***********************************************
    //The matrix in whic are stored the points is flattened inside d_pointsArray
    float* d_pointsArray;
    //***********************************************************************
    ...
    //CHANGE FROM 
    // ----------                    
    //| x1 | y1 |                        ________________________________________
    //| x2 | y2 | --------------------> | x1 | x2 | ... | xN | y1 | y2 | ... | yN
    //|....|....|                        ----------------------------------------
    //| N  | N  |
    cudaMalloc((void**)&d_pointsArray, DIM_HOST * DATASET_SIZE_HOST * sizeof(float));
    ...
    //loop to cover all the values, using an offset on the pointer
    for (int i = 0; i < DIM_HOST; i++)
        cudaMemcpy((d_pointsArray + (i * DATASET_SIZE_HOST)), pointsMatrix[i], DATASET_SIZE_HOST * sizeof(float), cudaMemcpyHostToDevice);


    //Piece of code for debugging the copy flattened of the dataset onto the GPU
    ...
    for (int iter = 0; iter < MAX_ITERATIONS; ++iter) {
        ...
        assignPointsToClusterKernel << <blocks, threads, 2 * K_HOST * sizeof(double) >> > (d_pointsArray, d_centroids, d_membership, d_sums, d_counts);
    ...
    //problema con deallocazione matrice
        // Free device memory
    cudaFree(d_pointsArray);
